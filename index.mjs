...
function deleteNote(title) {
  ...
  if (item) {
    ...
    console.log(
      `Note Title ${maintext(`'${item.title}'`)} has been deleted ${success(
        'successfully!'
      )}`
    );
  } else {
...
function usage() {
  ...
  console.log(
    `***Enter any of the following commands to start using Notes**\n\n ${info(
      'add, delete [title], list, leave'
    )}\n`
  );
  COMMANDS.forEach((cmd) => {
    let format;
    if (cmd.command === 'delete') {
      format = `${maintext(cmd.command)} ${maintext('[title]')}, ${info(
        `--${cmd.description}`
      )}\n`;
    } else {
...
function invalidCommand() {
  const msg = `${error('Invalid Command')}\n\ntype ${maintext(
    '--help'
  )} to see how to use this application.`;
  console.log(msg);
  resetConsole();
}
